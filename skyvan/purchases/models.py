...
def get_next_order_number():
    last_order = PurchaseOrder.objects.all().order_by("number").last()
    if last_order:
        return last_order.number + 1
    return 1


class PurchaseOrder(BaseModel, UserAuthorModel):
    uuid = models.UUIDField(primary_key=True, unique=True)
    supplier = models.ForeignKey(
        Supplier, on_delete=models.CASCADE, related_name="purchase_orders"
    )
    warehouse = models.ForeignKey(
        Warehouse, on_delete=models.CASCADE, related_name="purchase_warehouse"
    )
...
class PurchaseLine(BaseModel):
    ...
    product = models.ForeignKey(
        Product, related_name="purchase_lines", on_delete=models.CASCADE
    )
    ...
    def save(self, *args, **kwargs):
        # Ensure numeric fields are Decimal to avoid type errors
        self.quantity = (
        ...
        # Ensure purchase_order exists before calling method
