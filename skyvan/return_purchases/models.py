...
def get_next_order_number():
    last_order = ReturnPurchaseOrder.objects.all().order_by("number").last()
    if last_order:
        return last_order.number + 1
    return 1


class ReturnPurchaseOrder(BaseModel,UserAuthorModel):
    uuid = models.UUIDField(primary_key=True, unique=True)
    supplier = models.ForeignKey(
        Supplier, on_delete=models.CASCADE, related_name="return_purchase_orders"
    )
    warehouse = models.ForeignKey(
        Warehouse, on_delete=models.CASCADE, related_name="return_purchase_warehouse"
    )
    ...
    def __str__(self):
        return f"Order from {self.supplier.name} at {self.created_at.strftime('%Y-%m-%d')} in {self.warehouse.name} warehouse"
...
class ReturnPurchaseLine(BaseModel):
    uuid = models.UUIDField(primary_key=True, unique=True)
    return_purchase_order = models.ForeignKey(
        ReturnPurchaseOrder, related_name="lines", on_delete=models.CASCADE
    )
    product = models.ForeignKey(Product, related_name="return_purchase_lines",on_delete=models.CASCADE)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.return_purchase_order:
            self.return_purchase_order.recalculate_totals()
